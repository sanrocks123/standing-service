...
public class StandingPositionServiceImpl implements StandingPositionService {
    ...
    @Autowired private StandingPositionRepository positionRepo;

    /** */
    @Override
    public StandingPositionResponseDTO findPosition(final StandingPositionRequestDTO request) {

        final Optional<StandingCountryDTO> country =
                positionRepo.getCountries().stream()
                        .filter(c -> c.getCountryName().equalsIgnoreCase(request.getCountryName()))
                        .findAny();
        ...
        final Optional<StandingPositionResponseDTO> leagues =
                positionRepo.getLeaguesByCountryId(country.get().getCountryId()).stream()
                        .filter(l -> l.getLeagueName().equalsIgnoreCase(request.getLeagueName()))
                        .findAny();
        ...
        final Optional<StandingPositionResponseDTO> standing =
                positionRepo.getStandingsByLeagueId(leagues.get().getLeagueId()).stream()
                        .filter(
                                i ->
                                        i.getCountryName()
                                                        .equalsIgnoreCase(request.getCountryName())
                                                && i.getLeagueName()
                                                        .equalsIgnoreCase(request.getLeagueName())
                                                && i.getTeamName()
                                                        .equalsIgnoreCase(request.getTeamName()))
                        .findAny();
