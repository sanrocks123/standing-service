/**
  Standard CI/CD pipeline steps. Can be further extended to include automated test suites, etc
...
pipeline{
  ...
  stages{
    ...
    stage('Package'){
      // local docker daemon not installed, skipping docker image
    ...
    stage('Deploy'){
      // local docker daemon not installed, skipping deploy 
      steps {
         script { deploy('dev') }
       }
      }
